...
const allQueries = async (): Promise<void> => {
    ...
    const commentTable = `CREATE TABLE comment ( id INT NOT NULL AUTO_INCREMENT , comment TEXT NOT NULL , commentedOn DATETIME NOT NULL, commentedBy INT NOT NULL , logId INT NOT NULL, PRIMARY KEY (id), INDEX commentedBy (commentedBy), INDEX logId (logId), FOREIGN KEY (commentedBy) REFERENCES employee (id), FOREIGN KEY (logId) REFERENCES log (id))`
