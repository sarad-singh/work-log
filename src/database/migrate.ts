...
const allQueries = async (): Promise<void> => {
    ...
    const commentTable = `CREATE TABLE comment ( id INT NOT NULL AUTO_INCREMENT , comment TEXT NOT NULL , createdDate DATETIME NOT NULL, adminId INT NOT NULL , logId INT NOT NULL, PRIMARY KEY (id), INDEX adminId (adminId), INDEX logId (logId), FOREIGN KEY (adminId) REFERENCES employee (id), FOREIGN KEY (logId) REFERENCES log (id))`
