...
const logTable = "CREATE TABLE `work-log`.`log` ( `id` INT NOT NULL AUTO_INCREMENT , `title` VARCHAR(255) NOT NULL , `description` TEXT NOT NULL , `createdDate` DATETIME NOT NULL, `employeeId` INT NOT NULL , PRIMARY KEY (`id`), INDEX `emplyeeId` (`employeeId`), FOREIGN KEY (`employeeId`) REFERENCES `employee` (`id`))"
const commentTable = "CREATE TABLE `work-log`.`comment` ( `id` INT NOT NULL AUTO_INCREMENT , `comment` TEXT NOT NULL , `createdDate` DATETIME NOT NULL, `adminId` INT NOT NULL , `logId` INT NOT NULL, PRIMARY KEY (`id`), INDEX `adminId` (`adminId`), INDEX `logId` (`logId`), FOREIGN KEY (`adminId`) REFERENCES `employee` (`id`), FOREIGN KEY (`logId`) REFERENCES `log` (`id`))"
